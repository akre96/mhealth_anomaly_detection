...
def run_plots(inputs) -> None:
    ...
    try:
        ...
        exp = pd.read_csv(
            Path(CACHE_DIR, f'GLOBEM-{year}_{experiment}.csv')    
        )
    except FileNotFoundError:
        print(f'\tIntermediate files for GLOBEM Year {year} {experiment} do not exist in {CACHE_DIR}')
        return
    p_list = []

    print('\tCombining rolling windows in PHQ periods')
    features = [c for c in CHECK_MISSING_FEATURES if c in exp_ad.columns]
    print(f'\tUsing {len(features)} of {len(CHECK_MISSING_FEATURES)} features for checking missing days')
    for period in [1, 2, 3]:
    ...
    anoms = exp_ad.groupby(['subject_id', 'window_size'])[anomaly_detector_cols].sum().reset_index()
    ...
    for target in targets:
        ...
        if target == 'phq_stop':
            ...
            p1_corr.iloc[inds].to_csv(
                Path(out_dir, f'example_subs.csv'),
                index=False
            )
            for ind in inds:
                ...
                for i, (ws, w_df) in enumerate(exp_ad[exp_ad.subject_id == sid].groupby('window_size')):
        ...
        for metric in ['rho', 'r2']:
            ...
            for i, metric in enumerate(['rho', 'r2']):
                ...
                corr.groupby('model')[['rho', 'r2']].describe().round(3).to_csv(Path(out_dir, f'{target}_corr_model_summary.csv'))
            ax.set_ylabel('')
